...
const contextLoggerPlugin = context => {
    console.log(context);
    return context;
};
const lightningRunPlugin = context => {
...
const superSpeedPlugin = context => {
...
const gameEvents = ['start', 'pause', 'resume', 'end'];
gameEvents.forEach(event => game.on(event, params => console.log(event, params)));
game.start();
